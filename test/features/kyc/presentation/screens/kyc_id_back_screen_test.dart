...
class FakeKycRepository implements KycRepository {
  ...
  Future<KycDocument> captureDocument({required KycDocumentType type}) async {
    return KycDocument(id: 'doc-back', type: type, isUploaded: true);
  }
...
void main() {
  group('KycIdBackScreen', () {
    testWidgets('renders ID Card (Back) app bar title', (tester) async {
      await tester.pumpWidget(
        ProviderScope(
          ...
          child: const MaterialApp(home: KycIdBackScreen()),
        ),
      );
      await tester.pump();
    ...
    testWidgets('renders position instruction text', (tester) async {
      ...
      expect(
        find.textContaining('Position the back of your ID'),
        findsOneWidget,
      );
    });
