...
class FakeKycRepository implements KycRepository {
  ...
  Future<KycDocument> captureDocument({required KycDocumentType type}) async {
    return KycDocument(id: 'doc-front', type: type, isUploaded: true);
  }
...
void main() {
  group('KycIdFrontScreen', () {
    testWidgets('renders ID Card (Front) app bar title', (tester) async {
      await tester.pumpWidget(
        ProviderScope(
          ...
          child: const MaterialApp(home: KycIdFrontScreen()),
        ),
      );
      await tester.pump();
    ...
    testWidgets('renders position instruction text', (tester) async {
      ...
      expect(
        find.textContaining('Position the front of your ID'),
        findsOneWidget,
      );
    });
    ...
    testWidgets('shows Retake and Use this after capture', (tester) async {
      ...
      // Tap Capture and wait for async to complete
